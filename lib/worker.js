...
Worker.prototype.process = function (profiler, data, callback) {
    ...
    if (!func) {
        ...
        var workerProcess = getWorkerProcess(func, profiler, JSON.stringify(data.params), callback);
        workerProcess.send('run');
    }
};
...
function getWorkerProcess(func, profiler, params, callback) {
    var workerProcess = childProcess.fork('./worker_process', Array.from(arguments));

    workerProcess.on('message', function (processResult) {
        workerProcess.kill();
        return callback(null, processResult);
    });

    workerProcess.on('exit', function (code) {
        if (code > 0) {
            return callback(new Error('Worker died with code ' + code));
        }
    });  
